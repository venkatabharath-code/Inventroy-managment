...
const CreatePurchaseOrderModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // TC-19: Validate at least one item with a name is present
    ...
    try {
      ...
      setOrderItems([{ id: 1, item: '', qty: 1000, unit: 'Tablets', rate: 2.50, amount: 2500 }]);
      onClose();
  ...
  // Formatting helper
  const formatCurrency = (val) => `₹${val.toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`;

  return (
    <div className="cpo-overlay" onClick={onClose}>
      <form className="cpo-modal" onClick={e => e.stopPropagation()} onSubmit={handleSubmit}>
        
        {/* Header */}
        <div className="cpo-header">
          <h2>Create Purchase Order</h2>
          <button type="button" className="cpo-close" onClick={onClose} aria-label="Close modal" data-testid="cpo-close">
            <RiCloseLine />
          </button>
        </div>
                  ...
                  {orderItems.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="col-action">
                        <button type="button" className="btn-remove-item" onClick={() => handleRemoveItem(item.id)}>
